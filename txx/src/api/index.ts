...
const transform: AxiosTransform = {
  ...
  transformResponseHook: async (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    if (resData instanceof ArrayBuffer) {
      ...
      try {
        const json = JSON.parse(enc.decode(new Uint8Array(resData)));
        if (json.code === 500) {
          resData = { ...json };
        }
      } catch (_) {
    ...
    if (options.errorMessageMode === 'modal') {
      ...
      createMessage.error(t(timeoutMsg));
    }

    throw new Error(data.length > 0 ? t(timeoutMsg, { count: data[0] }) : t(timeoutMsg));
  },
    ...
    deepMerge(
      {
        ...
        headers: {
          ...
          'Local-Language': local?.locale || 'zh_CN',
        },
        // 如果是form-data格式
